...
TEXTURE* loadTextureFromFile(const char* filename)
{
    ...
    if(fread(header, 1, sizeof(header), texture_file) != sizeof(header))
        return nullptr;

    // PNG signature
    static const unsigned char png_sig[8] = {0x89, 'P', 'N', 'G', 0x0D, 0x0A, 0x1A, 0x0A};
    if(memcmp(header, png_sig, sizeof(png_sig)) == 0)
    {
        // Read full file into memory
        std::vector<unsigned char> file_data;
        file_data.reserve(1024);

        // We already read 8 bytes
        file_data.insert(file_data.end(), header, header + sizeof(header));

        unsigned char buf[4096];
        size_t r;
        while((r = fread(buf, 1, sizeof(buf), texture_file)) > 0)
            file_data.insert(file_data.end(), buf, buf + r);

        unsigned width = 0, height = 0;
        std::vector<unsigned char> rgba;
        unsigned err = lodepng::decode(rgba, width, height, file_data);
        if(err != 0)
            return nullptr;

        TEXTURE *texture = newTexture(width, height);
        if(!texture)
            return nullptr;
        ...
        for(unsigned y = 0; y < height; y++)
            for(unsigned x = 0; x < width; x++)
            {
                ...
                if(a < 16)
                    texture->bitmap[x + y * width] = 0;
                else
                    texture->bitmap[x + y * width] = (r & 0b11111000) << 8 | (g & 0b11111100) << 3 | (b & 0b11111000) >> 3;
            }

        return texture;
    }

    // Not PNG; fall back to existing PPM/PAM loader
