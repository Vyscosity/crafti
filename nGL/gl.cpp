...
void nglSetBuffer(COLOR *screenBuf, int width, int height)
{
    screen = screenBuf;
    screen_width = width;
...
void nglDisplay()
{
        ...
        if(!screen || !scr->pixels)
        {
            SDL_UnlockSurface(scr);
            return;
        }

        const int copyWidth = std::min<int>(screen_width, scr->w);
        const int copyHeight = std::min<int>(screen_height, scr->h);
        const int rowBytes = std::min<int>(copyWidth * sizeof(COLOR), scr->pitch);
        uint8_t *dst = static_cast<uint8_t*>(scr->pixels);
        for(int y = 0; y < copyHeight; ++y)
        {
            std::memcpy(dst + y*scr->pitch,
...
void nglDrawTriangleZClipped(const VERTEX *low, const VERTEX *middle, const VERTEX *high)
{
    ...
    // Keep projected vertices outside viewport and clip per-scanline in rasterization.
    nglDrawTriangleXZClipped(low, middle, high);
}
